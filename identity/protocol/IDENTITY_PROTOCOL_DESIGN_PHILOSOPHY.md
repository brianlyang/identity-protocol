...
Gateway-admissibility law is not a soft prose summary that may hide its machine rows
behind aggregate counts.
...
The machine world must not finalize gateway-admissibility legality while required
gateway identity drift remains known only internally.
...
Derivation law is not a soft prose summary that may hide its machine rows
behind aggregate counts.

Required derivation-class-profile family must remain explicit as a separate
machine-readable row family.

Expected row-family total and emitted row-family total must therefore stay
congruent under machine-readable coverage completeness rather than being left
implicit.

Expected row identity set and emitted row identity set for that family must
also remain machine-readable rather than being collapsed into aggregate counts
or generic structure failure.
...
Transition law is not a soft prose summary that may hide its machine rows
behind aggregate counts.

Required surface-class-profile, direct-root-target-edge, and
...
The machine world must not finalize transition legality while required
surface, promotion-edge, or re-entry-gateway identity drift remains known
only internally.
...
hide its machine rows behind aggregate counts.
...
Missing or unexpected question identity must remain projected in fail-close
...
Required commitment, anchor, primacy-proof, primacy-limit, and collapse
...
The machine world must not finalize machine-law primacy legality while
...
README root machine-law primacy completeness discipline must therefore stay
congruent with admitted machine-law-primacy-completeness rows rather than
becoming a freehand completeness summary.
...
Required strata, object, ontology-proof, ontology-limit, and collapse
families must remain explicit as separate machine-readable row families.
...
The machine world must not finalize machine-world ontology legality while
required row identity drift remains known only internally.
...
Agent-handoff law is not a soft prose summary that may hide its machine rows behind aggregate counts.
...
Expected row-family total and emitted row-family total must therefore stay congruent under machine-readable coverage completeness rather than being left implicit.

Expected row identity set and emitted row identity set for each family must also remain machine-readable rather than being collapsed into aggregate counts or generic structure failure.
...
Missing or unexpected row identity must remain projected in fail-close machine output rather than being hidden behind row-count shorthand or summary-only verdict text.
...
Operator answer-surface law is not a soft prose summary that may hide its
machine rows behind aggregate counts.
...
Identity-instance self-judgement law is not a soft prose summary that may hide
its machine rows behind aggregate counts.
...
The machine world must not finalize identity-instance self-judgement legality
...
Protocol-instance responsibility law is not a soft prose summary that may hide
its machine rows behind aggregate counts.
