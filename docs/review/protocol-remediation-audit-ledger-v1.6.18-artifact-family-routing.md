...
3. `scripts/ci/run_identity_dialogue_retention_probes_ci.sh` proves the bridge on a fixture pack and verifies coexistence with continuity tick/post-recover semantics.
   It now also proves the non-regression split between active current-thread binding and historical thread refresh: historical replay receipts can no longer hijack `current-thread.json`, and validator default receipt selection now follows the state-bound receipt before any `mtime`-latest fallback.
...
9. Current live proof breadth is no longer fixture-only:
   ...
   - `base-repo-closure-orchestrator` no longer carries the earlier inherited `rq_051_identity_dialogue_retention_contract_v1` residual after the shared dialogue-retention hardening made current-thread receipt binding explicit, blocked historical replay from clobbering active state, and replayed the protocol-owned guard runtime against the live pack runtime;
...
10. Current execution-closeout strengthening on live identities is now also proven through shared infrastructure rather than pack-local edits:
   ...
   - `scripts/tool_vendor_governance_common.py` now normalizes pack-scoped `identity/runtime/**` report contracts onto live runtime-pack roots instead of falling back to repo fixture/demo paths, so `base-repo-audit-expert-v3` now also replays `PASS_REQUIRED` on `tool_installation`, `vendor_api_discovery`, `vendor_api_solution`, and aggregate `validate_required_contract_coverage.py --operation scan` required coverage;
   - `base-repo-audit-expert-v3` therefore no longer carries protocol-owned required failures on either the adopted continuity/artifact-family subset or the previously misresolved discovery/tool/vendor subset, while `rq_052` stays green across all four active runtime identities after the live dialogue-retention resync on `base-repo-closure-orchestrator`.
...
2. architectural final lock is accepted: execution closeout owns the bounded whole-matrix machine path, live replay breadth, truth-sync maintenance, and any future inherited-family residual triage so long as it stays inside the frozen routing matrix;
