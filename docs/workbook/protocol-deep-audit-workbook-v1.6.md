...
- `current_evidence`:
  ...
  - direct runtime replay on `base-repo-audit-expert-v3` and `custom-creative-ecom-analyst` still remains `sample_report_only`; the open item is now real current-run producer coverage on applicable runtime identities, not missing shared machine-law semantics.
