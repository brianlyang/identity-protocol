...
- `current_evidence`:
  ...
  - `base-repo-closure-orchestrator` also currently replays `PASS_REQUIRED` on inherited `rq_051_identity_dialogue_retention_contract_v1`, so the previously recorded dialogue-retention residual is no longer current live truth;
