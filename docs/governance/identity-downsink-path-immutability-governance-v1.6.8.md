...
## 21) Control-plane scoped residue scan governance (2026-03-16)

### 21.1 Problem statement

1. Path residue checks occasionally scanned non-normative history trees and produced false P0 attribution (for example archival literals under backups/reports).
2. False positives blurred ownership boundary: protocol-side hardening vs instance-side runtime governance.

### 21.2 Mandatory behavior

1. Canonical residue scan script:
   - `scripts/scan_identity_path_residue.py`
2. Scan must be restricted to control-plane files only:
   - `catalog.local.yaml`
   - `CURRENT_TASK.json`
   - `IDENTITY_PROMPT.md`
   - `META.yaml`
   ...
   - `runtime/plugins/**/*`
   - `runtime/gate/**/*`
3. Scan must always exclude non-normative trees:
   - any `runtime/reports/**`
   - any `sanitization-backups/**`
   - any `*.bak*`
4. When scoped scan is clean (`PASS_REQUIRED`) but headstamp still drops on live traffic, default attribution is instance runtime governance unless protocol transport evidence shows a cross-instance control-plane bypass.

### 21.3 Required metrics

Each scan receipt must expose:
