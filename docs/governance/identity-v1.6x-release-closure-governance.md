...
12. `scripts/ci/run_release_readiness_summary_binding_probes_ci.sh` and `scripts/ci/run_release_readiness_continuation_probes_ci.sh` are additive machine-law freezes for the lifecycle/binding/continuation rules above and must remain green alongside `scripts/ci/run_terminal_truth_boundary_outer_surface_e2e_probes_ci.sh`.
   ...
   - `scripts/ci/run_three_plane_health_projection_probes_ci.sh` is the additive machine-law freeze for the new three-plane health companion surface and must keep proving full-surface pass, projection-only exclusion, and boundary-mismatch fail-close branches of `health_report_experience_writeback_closure`;
...
18. required-gate tuple parity must freeze the full explicit report authority tuple, not just the selected path token.
    - `scripts/required_gate_bundle_runner.py` now republishes bundle-level report authority companions (`report_selection_mode`, `report_selected_authority_class`, `report_pointer_resolution_mode`, `report_pointer_path`) whenever a strict surface passes `--report-selected-path`, instead of collapsing the bundle receipt back to a lone path string.
