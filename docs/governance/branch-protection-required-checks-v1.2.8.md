...
> 2) `identity/protocol/mappings/github-control-plane-offload.current.yaml` (protocol migration SSOT), and
> 3) `identity/protocol/mappings/stream-doc-registry.current.yaml` (active governance/review stream router).
