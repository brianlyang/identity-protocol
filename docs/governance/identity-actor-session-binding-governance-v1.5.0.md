...
| ASB-RQ-046 | protocol data sanitization boundary prevents tenant/business scenario leakage into protocol SSOT layer | `validate_protocol_data_sanitization_boundary` (new), governance/review document checks | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | false-positive hardening patch landed in `FIX-047` (phone-like regex path-context exemption + telemetry), but independent re-audit closure is still pending (`review 16.8.38`) |
...
| ASB-RQ-075 | protocol-layer feedback must use canonical reply-channel root and whitelist subpaths; non-standard primary channel is forbidden in strict operations | protocol-feedback reply writer/reader + channel classifier surfaces | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | implementation landed in `FIX-029` (`a95f5a2`) + remediation patch (`560f710`); current-state replay remains `PENDING_REAUDIT` in review (`16.8.28`, summary row `FIX-029`) |
| ASB-RQ-076 | required gate `validate_protocol_feedback_reply_channel.py` must enforce canonical primary channel + mirror-reference constraints across creator/e2e/readiness/full-scan/three-plane/CI | six-surface + CI required-gates wiring | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | validator + six-surface/CI wiring landed (`a95f5a2`) + CWD/fail-closed remediation (`560f710`); current-state replay remains `PENDING_REAUDIT` in review (`16.8.28`, summary row `FIX-029`) |
| ASB-RQ-077 | sidecar escalation blocking prefixes must include `IP-PFB-*` in strict operations so protocol-feedback channel violations become fail-closed | `validate_protocol_feedback_sidecar_contract.py` + readiness/CI sidecar adapter surfaces | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | strict blocking-prefix extension landed in `FIX-029` (`a95f5a2`) with remediation chain (`560f710`); independent closure still pending in review (`16.8.28`) |
| ASB-RQ-078 | split-receipt requiredization bridge must auto-promote split contract to required when protocol-feedback activity exists; strict operations must not return `SKIPPED_NOT_REQUIRED` in this case | split-receipt validator + requiredization bridge + strict lane orchestration surfaces | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | activity-aware requiredization bridge + `IP-PFB-CH-006` path landed (`a95f5a2`) with strict replay remediation (`560f710`); current-state replay remains `PENDING_REAUDIT` (`16.8.28`) |
| ASB-RQ-079 | protocol-lane entry must be trigger-auditable and bootstrap-ready: when `work_layer=protocol` / `protocol_triggered=true`, canonical protocol-feedback roots must be ready before protocol conclusions are emitted | layer-intent/send-time/reply bridge + bootstrap readiness classifier surfaces | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | bootstrap-readiness classifier/constructor landed in `FIX-030` (`a95f5a2`) + remediation chain (`560f710`); current-state replay remains `PENDING_REAUDIT` in review (`16.8.28`, summary row `FIX-030`) |
| ASB-RQ-080 | required gate `validate_protocol_feedback_bootstrap_ready.py` must enforce canonical root readiness (`outbox-to-protocol`, `evidence-index`, `upgrade-proposals`) and emit machine-readable bootstrap telemetry | creator/readiness/e2e/full-scan/three-plane/CI wiring | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | required gate + telemetry + six-surface/CI wiring landed (`a95f5a2`) with deterministic bootstrap remediation (`560f710`); current-state replay remains `PENDING_REAUDIT` (`16.8.28`) |
| ASB-RQ-081 | strict operations must fail-closed (`IP-PFB-CH-004/005`) when protocol lane is selected but bootstrap readiness or SSOT linkage is missing | strict-lane adapters + blocker-receipt integration + scan/report telemetry surfaces | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | strict fail-closed semantics landed (`a95f5a2`) + replay remediation (`560f710`); independent closure still pending in review (`16.8.28`) |
| ASB-RQ-082 | layer-intent resolver must expose `PROTOCOL_CANDIDATE` decision state so weak protocol concern can enter governed clarification flow instead of silent instance fallback | layer-intent classifier + reply/send-time bridge surfaces | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | candidate decision-state bridge landed in `FIX-031` (`a95f5a2`) + remediation (`560f710`); current-state replay remains `PENDING_REAUDIT` in review (`16.8.28`, summary row `FIX-031`) |
| ASB-RQ-083 | required gate `validate_protocol_entry_candidate_bridge.py` must enforce candidate clarification receipt completeness and deterministic question set (`which_gate_or_stage_failed`, `latest_replay_or_evidence_path`, `expected_protocol_optimization_target`) | creator/readiness/e2e/full-scan/three-plane/CI wiring | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | required gate + deterministic question-set enforcement landed (`a95f5a2`) with routing/cwd remediation (`560f710`); closure still pending in review (`16.8.28`) |
| ASB-RQ-084 | candidate protocol-entry flow must seed canonical protocol-feedback outbox/index before final protocol conclusion; missing seed/index linkage is strict fail-closed (`IP-LAYER-CAND-003/004`) | candidate seed writer + SSOT archival bridge + strict-lane adapters | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | canonical seed/index linkage enforcement landed (`a95f5a2`) + remediation chain (`560f710`); independent closure pending (`16.8.28`) |
| ASB-RQ-085 | strict operations must fail-closed on silent candidate downgrade without clarification evidence (`IP-LAYER-CAND-001`) and expose candidate promotion telemetry in three-plane/full-scan | strict-lane adapters + scan/report telemetry surfaces | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | anti-silent-downgrade semantics + telemetry landed (`a95f5a2`) with remediation chain (`560f710`); current-state replay remains `PENDING_REAUDIT` (`16.8.28`) |
| ASB-RQ-086 | weak protocol concerns must enter deterministic inquiry follow-up states (`QUESTION_REQUIRED -> EVIDENCE_PENDING -> READY_FOR_PROTOCOL_FEEDBACK`) instead of silent downgrade/drop | layer-intent classifier + candidate/inquiry adapters + inquiry receipt writer surfaces | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | inquiry state machine landed in `FIX-032` (`a95f5a2`) + remediation (`560f710`); current-state replay remains `PENDING_REAUDIT` in review (`16.8.28`, summary row `FIX-032`) |
| ASB-RQ-087 | inquiry receipts must classify `signal_origin` and require sanitized paraphrase before promoting business-origin statements into protocol conclusions | inquiry classifier + sanitization receipt writer + protocol conclusion adapters | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | signal-origin classification + sanitization enforcement landed (`a95f5a2`) with remediation chain (`560f710`); closure still pending in review (`16.8.28`) |
| ASB-RQ-088 | `source` (identity provenance) and `source_layer` (layer-intent provenance) must remain machine-distinct and mapper-auditable; ambiguity must not silently alter `work_layer` | stamp parser/coherence validator + layer-intent resolver + telemetry surfaces | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | semantic distinction logic landed via `FIX-032` (`a95f5a2`) with remediation (`560f710`); independent closure pending in review (`16.8.28`) |
| ASB-RQ-089 | required gate `validate_protocol_inquiry_followup_chain.py` must fail-closed on missing follow-up receipts, missing protocol-feedback linkage, unsanitized promotion, and stale/expired inquiry chains (`IP-LAYER-INQ-001..004`) across creator/readiness/e2e/full-scan/three-plane/CI | six-surface + required-gates workflow wiring + requiredization trigger bridge | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | required gate + six-surface/CI wiring + requiredization trigger bridge landed (`a95f5a2`) with emission-correlation remediation (`560f710`); current-state replay remains `PENDING_REAUDIT` (`16.8.28`) |
...
| ASB-RQ-107 | identity scaffold default bootstrap must be domain-neutral and path-governed: no legacy business constants in default generated `CURRENT_TASK`/runtime samples, and no cross-root relative pointers | `scripts/create_identity_pack.py` + neutral scaffold seed artifacts + scaffold-neutrality validator (new) + creator/readiness scan telemetry surfaces | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | implementation landed in `FIX-048` (`f5c97b3`) with neutral default scaffold + explicit legacy overlay opt-in; replay intake recorded in review `16.8.39B`, independent re-audit closure pending |
...
| ASB-RQ-111 | post-execution strict closure must be CWD-invariant: relative writeback/runtime-state paths must resolve deterministically against report/pack anchors, and child validator invocation must not depend on caller working directory | `validate_identity_experience_writeback.py`, `validate_identity_prompt_lifecycle.py`, `validate_post_execution_mandatory.py` path canonicalization + absolute script invocation adapter | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | implementation landed in `FIX-051` (`e62deab`, `ddb1529`, `6430852`) with pack/report-root canonicalization + absolute validator invocation chain (including writeback path-contract child validator); targeted replay closure is recorded in review `16.8.46/16.8.47`. latest live project replay keeps `IP-CAP-003` as env/auth telemetry boundary via fallback trace and scan fallback evidence (`review 16.8.51/16.8.52`), and independent closure audit remains pending |
