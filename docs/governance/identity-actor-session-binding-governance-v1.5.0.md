...
| D6 Tag allowed | D1~D5 all PASS + unlock formula (6.5) satisfied | PASS (UNLOCK_ALLOWED; snapshot `6.4J`) |
...
1. OpenAI Codex Skills guidance defines skills as reusable instruction packages with progressive disclosure (`metadata -> SKILL.md -> references/scripts as needed`), which implies scaffold defaults should stay generic and not preload domain-specific constants.
2. Agent Skills specification requires minimal portable frontmatter (`name`, `description`) and optional assets/scripts; therefore default bootstrap artifacts must preserve portability across domains.
3. MCP specification models capabilities by typed boundaries (`tools`, `resources`, `prompts`) and capability declaration, not by vertical-business constants; scaffold bootstrap must avoid embedding commerce-specific route tokens as implicit capability selectors.
4. Security/consent alignment:
   - when a workflow truly requires vendor/domain specialization, specialization must be explicit and auditable (profile opt-in + receipt), not hidden in default scaffold seeds.
...
Canonical blocker baseline (migration target):

1. `auth_login_required`
2. `anti_automation_challenge_required`
3. `session_reauthentication_required`
4. `manual_verification_required`

Legacy bridge requirements:

1. Legacy aliases (for example `login_required`, `captcha_required`, `session_expired`) are compatibility inputs only.
2. Strict lanes MUST emit alias-hit telemetry and resolved canonical blocker.
3. Bridge deprecation window must be explicit in task/runtime metadata; missing window metadata in strict lanes is fail-closed.
...
| --- | --- | --- | --- | --- | --- |
| ASB-RQ-001 | actor session binding contract schema + storage contract (`session/actors/<actor_id>.json`) | `resolve_identity_context.py`, activation/session pointer sync routine | P0 | DONE | actor-scoped canonical source + mirror compatibility + six-surface replay are closed (`FIX-009/010/015`, review `16.8.74`). |
| ASB-RQ-002 | dual-write migration (actor pointer + legacy mirror) | activation/session pointer mirror routine, migration utilities | P0 | DONE | dual-write migration and compatibility replay are closed in actor-scoped chain (`FIX-009/010/015`, review `16.8.74`). |
| ASB-RQ-003 | validator: actor session binding integrity | `validate_actor_session_binding` (new) | P0 | DONE | validator landed and replay-closed in required lanes (`FIX-010`, `HOTFIX-P0-002`, review `16.8.74`). |
| ASB-RQ-004 | validator: no implicit switch across actors | `validate_no_implicit_switch` (new) | P0 | DONE | validator landed and replay-closed in required lanes (`FIX-010`, `HOTFIX-P0-002`, review `16.8.74`). |
| ASB-RQ-005 | validator: cross-actor isolation | `validate_cross_actor_isolation` (new) | P0 | DONE | validator landed and replay-closed in required lanes (`FIX-010`, `HOTFIX-P0-002`, review `16.8.74`). |
| ASB-RQ-006 | dynamic response stamp renderer (redacted external view + internal evidence view) | `render_identity_response_stamp` (new) | P0 | DONE | renderer/dual-view safety boundary is implementation+replay closed (`FIX-004`, `HOTFIX-P0-001/003`, `FIX-020`). |
| ASB-RQ-007 | response stamp validator + CI enforcement | `validate_identity_response_stamp` (new), required-gates | P0 | DONE | validator + required-gate wiring are replay closed (`HOTFIX-P0-003/004`, `FIX-024`, `FIX-054`). |
...
| ASB-RQ-009 | mandatory gate wiring across creator/e2e/readiness/full-scan/three-plane/CI | multiple surfaces listed in 6.3 | P0 | DONE | mandatory wiring is closed across creator/e2e/readiness/full-scan/three-plane/CI (`FIX-009/010/015`, `HOTFIX-P0-002`). |
| ASB-RQ-010 | single-active removal from activation/consistency path | `identity_creator.py`, `identity_installer.py`, `validate_identity_state_consistency.py`, `validate_identity_session_pointer_consistency.py`, `compile_identity_runtime.py` | P0 | DONE | actor-scoped multi-active patch is committed and replay-audited (`FIX-015`) |
...
| ASB-RQ-020 | external stamp output is redacted by default; absolute paths reserved for internal evidence view | `render_identity_response_stamp`, `validate_identity_response_stamp` | P0 | DONE | redacted external/internal evidence view boundary is replay closed (`FIX-004`, `HOTFIX-P0-001/003`, `FIX-054`). |
| ASB-RQ-021 | stamp render/validate must be CWD-invariant and deterministic across project/global pointer arbitration | stamp renderer + stamp validator + readiness/e2e/full-scan/three-plane/CI surfaces | P0 | DONE | CWD-invariant stamp render/validate surfaces are replay closed (`FIX-024`, `FIX-056`, review `16.8.68`). |
...
| ASB-RQ-028 | runtime catalog `pack_path` canonicalization is strict and machine-validated | `validate_identity_pack_path_canonical` (new), resolver/catalog write paths | P0 | DONE | path-governance chain is implementation+replay closed (`FIX-002~007`, `FIX-056`). |
| ASB-RQ-029 | report `resolved_pack_path` is canonical absolute and non-relative | `validate_identity_execution_report_path_contract` (new), `execute_identity_upgrade.py` | P0 | DONE | execution-report path contract is implementation+replay closed (`FIX-002~007`, `FIX-051`). |
| ASB-RQ-030 | `identity_home == dirname(identity_catalog)` alignment is fail-closed for runtime mutation | `validate_identity_home_catalog_alignment` (new), runtime mode guard surfaces | P0 | DONE | home/catalog alignment fail-closed semantics are replay closed in path-governance chain (`FIX-002~007`). |
| ASB-RQ-031 | fixture/runtime boundary is validator-enforced for activate/update/readiness paths | `validate_fixture_runtime_boundary` (new), creator/update/readiness/scan surfaces | P0 | DONE | fixture/runtime boundary validator is implementation+replay closed (`FIX-002~007`, `FIX-057`). |
| ASB-RQ-032 | writeback continuity contract enforces strict/degraded writeback semantics with mandatory risk fields | `validate_writeback_continuity` (new), `execute_identity_upgrade.py`, readiness/e2e/scan surfaces | P0 | DONE | writeback continuity gate is implementation+replay closed (`FIX-011`, `FIX-051`). |
| ASB-RQ-033 | post-execution mandatory validator blocks closure when runtime state machine is not advanced | `validate_post_execution_mandatory` (new), upgrade execution closure surfaces | P0 | DONE | post-execution mandatory gate is implementation+replay closed (`FIX-011`, `FIX-051`). |
| ASB-RQ-034 | semantic routing guard contract enforces intent-domain pre-classification before retrieval | `validate_semantic_routing_guard` (new), protocol-feedback router surfaces | P0 | DONE | semantic routing guard is implementation+replay closed (`FIX-012`, `FIX-052`). |
| ASB-RQ-035 | protocol-vendor and business-partner namespace separation is validator-enforced | `validate_vendor_namespace_separation` (new), feedback artifact writers/readers | P0 | DONE | namespace separation guard is implementation+replay closed (`FIX-013`, `FIX-041/042`). |
| ASB-RQ-036 | protocol-feedback sidecar escalation remains auditable and non-blocking-by-default | `validate_protocol_feedback_sidecar_contract` (new), required-gates escalation logic | P0 | DONE | sidecar escalation contract is implementation+replay closed (`FIX-013`, `FIX-051` sidecar ordering closure). |
| ASB-RQ-037 | instance-to-base-repo mutation boundary is codified as docs-allow/protocol-code-deny and fail-closed | `validate_instance_base_repo_write_boundary` (new), readiness/e2e/CI change-range surfaces | P0 | DONE | replayed and audit-confirmed in HOTFIX-P0-005 lane |
| ASB-RQ-038 | protocol-feedback outputs must be SSOT-archived before mirror publication | `validate_protocol_feedback_ssot_archival` (new), outbox/evidence-index writer surfaces | P0 | DONE | replayed and audit-confirmed in HOTFIX-P0-006 lane |
| ASB-RQ-039 | readiness dual-catalog arbitration must be explicit via `--scope` and deterministic fail-closed on ambiguity | `release_readiness_check.py` + runtime mode/scope validators (`validate_identity_runtime_mode_guard`) | P0 | DONE | no-scope fail-closed + scoped passthrough replayed and confirmed in HOTFIX-P0-007 lane |
| ASB-RQ-040 | user-facing reply stamp presence must be machine-counted in replay outputs (`reply_stamp_missing_count`) | `validate_identity_response_stamp` (coverage mode), three-plane/full-scan replay surfaces | P0 | DONE | user-visible zero-miss closure is audit-passed (`HOTFIX-P0-004`) |
| ASB-RQ-041 | readiness explicit scope must propagate into health branch validators (`collect_identity_health_report` path) | `release_readiness_check.py`, `collect_identity_health_report.py` | P0 | DONE | implemented and audit-passed (`FIX-017`) |
| ASB-RQ-042 | baseline policy must be stratified (`strict` for release/mutation, `warn` for observability-only paths) | readiness/e2e/creator/scan/three-plane baseline policy wiring | P0 | DONE | implemented and audit-passed (`FIX-018`) |
| ASB-RQ-043 | protocol version alignment must be validated as unified tuple across report/prompt/task/binding context | baseline/prompt/binding validators + closure surfaces | P0 | DONE | unified validator implemented and audit-passed (`FIX-019`) |
| ASB-RQ-044 | protocol-vendor semantic isolation validator blocks cross-domain pollution in conclusion layer | `validate_protocol_vendor_semantic_isolation` (new), protocol-feedback conclusion writer surfaces | P0 | DONE | implemented and audit-passed (`P0-D`) |
| ASB-RQ-045 | external source trust chain validator enforces trusted source tiers for conclusion-layer evidence | `validate_external_source_trust_chain` (new), external retrieval/evidence aggregation surfaces | P0 | DONE | implemented and audit-passed (`P0-E`) |
| ASB-RQ-046 | protocol data sanitization boundary prevents tenant/business scenario leakage into protocol SSOT layer | `validate_protocol_data_sanitization_boundary` (new), governance/review document checks | P0 | DONE | false-positive hardening patch landed in `FIX-047` (`d50b3a9`); independent replay closure promoted in review `16.8.50`. |
...
| ASB-RQ-054 | user-visible identity stamp hard gate must fail-closed when actor/session lock is not `LOCK_MATCH` in strict operations (prevent perceived hard-switch under dual-catalog drift) | `validate_identity_response_stamp.py`, `validate_reply_identity_context_first_line.py`, creator/readiness/e2e wiring | P0 | DONE | implementation + strict/inspection replay audit-passed (`HOTFIX-P0-008` / `FIX-020`) |
| ASB-RQ-055 | per-round instance/protocol split receipt is mandatory and machine-readable (`split_notice`, `instance_actions`, `protocol_actions`, `feedback_triggered`, `evidence_index`) | runtime reply receipt schema + replay parsers + three-plane/full-scan visibility | P0 | DONE | implemented via `validate_instance_protocol_split_receipt.py` + replay payload mapping (`8778bdf`) |
| ASB-RQ-056 | required gate `validate_instance_protocol_split_receipt.py` enforces split fields + trigger semantics + SSOT path linkage | creator/readiness/e2e/full-scan/three-plane/CI wiring | P0 | DONE | six-surface wiring landed (`8778bdf`) |
| ASB-RQ-057 | protocol-feedback trigger must follow hard conditions (recurrence, false-green/red, evidence missing, lane contamination) | trigger classifier + receipt validator + protocol-feedback archival bridge | P0 | DONE | hard-condition classifier + IP-SPLIT failure paths landed (`8778bdf`) |
| ASB-RQ-058 | dual-track section isolation (`instance` vs `protocol`) must be enforced and protocol lane must remain business-data sanitized | receipt parser + `validate_protocol_data_sanitization_boundary.py` extension | P0 | DONE | anti-mixed-lane + sanitization checks landed in split-receipt validator (`8778bdf`) |
| ASB-RQ-059 | trigger-regression sample report path patterns must resolve against identity pack root, never process CWD | `validate_identity_trigger_regression.py` path resolver + replay fixtures | P0 | DONE | CWD-invariant resolution landed and replayed from `/tmp` (`8778bdf`) |
| ASB-RQ-060 | handoff self-test sample roots must be pack-root anchored for positive/negative fixture discovery | `validate_agent_handoff_contract.py` self-test sample root resolver | P0 | DONE | pack/protocol-root deterministic fallback landed (`8778bdf`) |
| ASB-RQ-061 | orchestration subprocess and repo-catalog resolution must be protocol-root deterministic across invocation directories | `report_three_plane_status.py` subprocess launcher + repo-catalog resolver (`--protocol-root` aware) | P0 | DONE | protocol-root deterministic launch + IP-CWD-004 error semantics landed (`8778bdf`) |
| ASB-RQ-062 | discovery requiredization trigger classifier must deterministically promote discovery contracts from optional to required when risk conditions are met | requiredization state evaluator + CURRENT_TASK contract writeback surfaces | P0 | DONE | `validate_discovery_requiredization.py` + update preflight apply path landed (`295daf7`,`3baa355`) |
| ASB-RQ-063 | `validate_discovery_requiredization.py` must fail-closed when trigger met but requiredization/writeback/SSOT linkage is missing | validator + creator/readiness/e2e/full-scan/three-plane/CI wiring | P0 | DONE | IP-DREQ-001..004 fail-closed + six-surface wiring landed (`295daf7`,`3baa355`) |
| ASB-RQ-064 | CI required validator set must auto-include discovery trio when requiredization is active | `ci_enforcement_contract.required_validators` synchronizer + CI required-gates adapter | P0 | DONE | apply-requiredization path auto-syncs CI required validators (`3baa355`) |
...
| ASB-RQ-066 | required-contract coverage must expose discovery-subset hard threshold for requiredized contracts | `validate_required_contract_coverage.py` discovery subset counters + `min_discovery_required_coverage` gate | P0 | DONE | discovery subset counters + threshold gate landed (`295daf7`) |
| ASB-RQ-067 | strict operations must enforce execution-to-reply identity tuple coherence under dual-catalog lanes | `execution_reply_identity_coherence_contract_v1` + coherence validator/wiring across creator/readiness/e2e/full-scan/three-plane/CI | P0 | DONE | independent replay closure confirmed in review `16.8.53` (`FIX-021` promoted to `PASS`: strict pass/fail + inspection warn paths all matched expected semantics). |
| ASB-RQ-068 | send-time unified reply outlet gate must enforce first-line Identity-Context fail-closed semantics and emit machine-readable telemetry in three-plane/full-scan | `validate_send_time_reply_gate.py` + creator/readiness/e2e/full-scan/three-plane/CI wiring | P0 | DONE | independent replay closure confirmed in review `16.8.53` (`FIX-024` promoted to `PASS`: reply-file positive + missing-header fail-closed `IP-ASB-STAMP-SESSION-001`). |
| ASB-RQ-069 | layer intent resolution must auto-resolve `work_layer/source_layer` with confidence + fallback telemetry and keep strict tuple fail-closed semantics | `resolve_layer_intent` + `validate_layer_intent_resolution.py` + render/readiness/e2e/full-scan/three-plane/CI wiring | P1 | GATE_READY (NON_BLOCKING) | independent replay closure confirmed in review `16.8.53` (`FIX-025/026` promoted to `PASS`: instance/protocol/ambiguous samples + pass-through telemetry across send-time/full-scan/three-plane). |
| ASB-RQ-070 | default work layer must be `instance`, and protocol escalation must be trigger-auditable with deterministic sample regression gate | `default_work_layer_contract_v1` + `protocol_trigger_contract_v1` + `layer_consistency_gate` in resolver/validators (render + first-line + coherence + send-time + layer-intent regression samples) | P0 | DONE | independent replay closure confirmed in review `16.8.53` (`FIX-027` promoted to `PASS`: default instance path + protocol-triggered path + regression sample set all passed). |
| ASB-RQ-071 | canonical actor binding payload must be multi-entry and key-safe for same-actor concurrent sessions (`actor_id + session_id`), forbidding single-record overwrite semantics in strict lanes | actor binding serializer/reader + migration shims (`session/actors/<actor>.json`) | P0 | DONE | independent replay closure confirmed in review `16.8.53` (`FIX-028` promoted to `PASS`: positive multibinding path and `IP-ASB-MB-001` negative replay matched). |
| ASB-RQ-072 | actor binding mutation must enforce CAS-style precondition and fail-closed on stale compare token | activation binding writer + validator | P0 | DONE | independent replay closure confirmed in review `16.8.53` (`FIX-028` promoted to `PASS`: CAS conflict replay emitted `IP-ASB-MB-003`). |
| ASB-RQ-073 | canonical actor binding mutation must be activation-only and append-only rebind receipt must be mandatory | creator/readiness/scan mutation boundary + receipt writer surfaces | P0 | DONE | independent replay closure confirmed in review `16.8.53` with strict mutation-boundary/receipt semantics retained under multibinding contract (`FIX-028`). |
| ASB-RQ-074 | required gate `validate_actor_session_multibinding_concurrency.py` must be wired across creator/e2e/readiness/full-scan/three-plane/CI and expose machine-readable conflict telemetry | six-surface + CI required-gates wiring | P0 | DONE | independent replay closure confirmed in review `16.8.53` (`FIX-028` summary row backfilled to commit `48e5445`, status promoted to `PASS`). |
| ASB-RQ-075 | protocol-layer feedback must use canonical reply-channel root and whitelist subpaths; non-standard primary channel is forbidden in strict operations | protocol-feedback reply writer/reader + channel classifier surfaces | P0 | DONE | implementation + remediation landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-029`). |
| ASB-RQ-076 | required gate `validate_protocol_feedback_reply_channel.py` must enforce canonical primary channel + mirror-reference constraints across creator/e2e/readiness/full-scan/three-plane/CI | six-surface + CI required-gates wiring | P0 | DONE | validator + six-surface/CI wiring landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-029`). |
| ASB-RQ-077 | sidecar escalation blocking prefixes must include `IP-PFB-*` in strict operations so protocol-feedback channel violations become fail-closed | `validate_protocol_feedback_sidecar_contract.py` + readiness/CI sidecar adapter surfaces | P0 | DONE | blocking-prefix hardening landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-029`). |
| ASB-RQ-078 | split-receipt requiredization bridge must auto-promote split contract to required when protocol-feedback activity exists; strict operations must not return `SKIPPED_NOT_REQUIRED` in this case | split-receipt validator + requiredization bridge + strict lane orchestration surfaces | P0 | DONE | requiredization bridge + strict path landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-029`). |
| ASB-RQ-079 | protocol-lane entry must be trigger-auditable and bootstrap-ready: when `work_layer=protocol` / `protocol_triggered=true`, canonical protocol-feedback roots must be ready before protocol conclusions are emitted | layer-intent/send-time/reply bridge + bootstrap readiness classifier surfaces | P0 | DONE | bootstrap-readiness classifier/constructor landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-030`). |
| ASB-RQ-080 | required gate `validate_protocol_feedback_bootstrap_ready.py` must enforce canonical root readiness (`outbox-to-protocol`, `evidence-index`, `upgrade-proposals`) and emit machine-readable bootstrap telemetry | creator/readiness/e2e/full-scan/three-plane/CI wiring | P0 | DONE | required gate + telemetry + six-surface wiring landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-030`). |
| ASB-RQ-081 | strict operations must fail-closed (`IP-PFB-CH-004/005`) when protocol lane is selected but bootstrap readiness or SSOT linkage is missing | strict-lane adapters + blocker-receipt integration + scan/report telemetry surfaces | P0 | DONE | strict fail-closed semantics landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-030`). |
| ASB-RQ-082 | layer-intent resolver must expose `PROTOCOL_CANDIDATE` decision state so weak protocol concern can enter governed clarification flow instead of silent instance fallback | layer-intent classifier + reply/send-time bridge surfaces | P0 | DONE | candidate decision-state bridge landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-031`). |
| ASB-RQ-083 | required gate `validate_protocol_entry_candidate_bridge.py` must enforce candidate clarification receipt completeness and deterministic question set (`which_gate_or_stage_failed`, `latest_replay_or_evidence_path`, `expected_protocol_optimization_target`) | creator/readiness/e2e/full-scan/three-plane/CI wiring | P0 | DONE | required gate + deterministic question-set enforcement landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-031`). |
| ASB-RQ-084 | candidate protocol-entry flow must seed canonical protocol-feedback outbox/index before final protocol conclusion; missing seed/index linkage is strict fail-closed (`IP-LAYER-CAND-003/004`) | candidate seed writer + SSOT archival bridge + strict-lane adapters | P0 | DONE | canonical seed/index linkage enforcement landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-031`). |
| ASB-RQ-085 | strict operations must fail-closed on silent candidate downgrade without clarification evidence (`IP-LAYER-CAND-001`) and expose candidate promotion telemetry in three-plane/full-scan | strict-lane adapters + scan/report telemetry surfaces | P0 | DONE | anti-silent-downgrade semantics + telemetry landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-031`). |
| ASB-RQ-086 | weak protocol concerns must enter deterministic inquiry follow-up states (`QUESTION_REQUIRED -> EVIDENCE_PENDING -> READY_FOR_PROTOCOL_FEEDBACK`) instead of silent downgrade/drop | layer-intent classifier + candidate/inquiry adapters + inquiry receipt writer surfaces | P0 | DONE | inquiry state machine landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-032`). |
| ASB-RQ-087 | inquiry receipts must classify `signal_origin` and require sanitized paraphrase before promoting business-origin statements into protocol conclusions | inquiry classifier + sanitization receipt writer + protocol conclusion adapters | P0 | DONE | signal-origin classification + sanitization enforcement landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-032`). |
| ASB-RQ-088 | `source` (identity provenance) and `source_layer` (layer-intent provenance) must remain machine-distinct and mapper-auditable; ambiguity must not silently alter `work_layer` | stamp parser/coherence validator + layer-intent resolver + telemetry surfaces | P0 | DONE | semantic distinction logic landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-032`). |
| ASB-RQ-089 | required gate `validate_protocol_inquiry_followup_chain.py` must fail-closed on missing follow-up receipts, missing protocol-feedback linkage, unsanitized promotion, and stale/expired inquiry chains (`IP-LAYER-INQ-001..004`) across creator/readiness/e2e/full-scan/three-plane/CI | six-surface + required-gates workflow wiring + requiredization trigger bridge | P0 | DONE | required gate + six-surface/CI wiring + requiredization trigger bridge landed (`a95f5a2`,`560f710`); independent replay closure promoted in review `16.8.50` (`FIX-032`). |
| ASB-RQ-090 | work-layer must deterministically route to lane-specific required gate set (`instance_required_checks` vs `protocol_required_checks`) with strict mismatch fail-closed semantics | readiness/e2e/creator/full-scan/three-plane gate orchestration routers + lane tuple receipts | P0 | DONE | `FIX-033` implementation landed (`0d7ebc7`) + lane applied-gate-set propagation patch (`913973a`); independent replay closed in review (`16.8.27`) |
| ASB-RQ-091 | `work_layer=instance` self-drive upgrades must not be hard-blocked by protocol publish gates (e.g., changelog/release metadata); protocol diffs emit side-channel protocol-feedback receipt instead of blocking | readiness/e2e/creator lane filter + protocol-feedback pending receipt writer | P0 | DONE | `FIX-033` implementation landed (`0d7ebc7`) + re-audit docs closure (`9d830d8`); independent replay closed in review (`16.8.27`) |
| ASB-RQ-092 | `work_layer=protocol` must enforce protocol publish governance gates and canonical protocol-feedback closure (`runtime/protocol-feedback/...`) as fail-closed boundaries | protocol lane required gate set + canonical outbox/index/upgrade proposal validators + strict blocker receipts | P0 | DONE | `FIX-033` implementation landed (`0d7ebc7`) + lane applied-gate-set replay closure (`913973a`,`9d830d8`); independent replay closed in review (`16.8.27`) |
| ASB-RQ-093 | send-time/replay telemetry must expose lane execution proof (`work_layer`, `applied_gate_set`, `protocol_feedback_triggered`, `protocol_feedback_paths`, `lane_transition_reason`) for each governed round | response-stamp tail + first-line/send-time validators + full-scan/three-plane payload mapping | P0 | DONE | telemetry closure verified by independent replay after patch (`913973a`,`9d830d8`), review record `16.8.27`; additional scope-limited instance Full-Go evidence intake is recorded in review `16.8.42` |
| ASB-RQ-094 | protocol-topic sessions must not silently downgrade to `work_layer=instance` on empty intent fallback; strict operations require explicit protocol lane confirmation or session lane lock evidence | lane-intent resolver + readiness/e2e/creator strict gate routing + session-lane lock receipt surfaces | P0 | DONE | `FIX-034` landed in `c310ab4`; independent replay closure promoted in review `16.8.50`. |
| ASB-RQ-095 | strict baseline freshness must evaluate against run-pinned protocol SHA to avoid in-run nondeterministic failures caused by moving HEAD | baseline/session-refresh validators + run context pin writer + readiness/e2e preflight adapters | P0 | DONE | `FIX-035` landed in `c310ab4`; independent replay closure promoted in review `16.8.50`. |
| ASB-RQ-096 | e2e runner must be hermetic and pass without external `PYTHONPATH` preparation | `e2e_smoke_test.sh` import-path bootstrap + hermetic preflight validator + CI replay adapter | P0 | DONE | `FIX-036` landed in `c310ab4`; independent replay closure promoted in review `16.8.50`. |
...
| ASB-RQ-104 | protocol lane lock exit must have a unified writer entrypoint with canonical outbox + index linkage; strict lane exit without newer EXIT remains fail-closed | `write_session_lane_lock_exit.py` + `identity_creator.py update --release-session-lane-lock` + lane-routing exit telemetry surfaces | P0 | DONE | `FIX-044` landed in `62bdc1c` with canonical EXIT writer and creator automation switch; independent replay closure promoted in review `16.8.50` (with scope-limited evidence addendum in `16.8.42`). |
...
| ASB-RQ-107 | identity scaffold default bootstrap must be domain-neutral and path-governed: no legacy business constants in default generated `CURRENT_TASK`/runtime samples, and no cross-root relative pointers | `scripts/create_identity_pack.py` + neutral scaffold seed artifacts + scaffold-neutrality validator (new) + creator/readiness scan telemetry surfaces | P0 | DONE | implementation landed in `FIX-048` (`f5c97b3`,`49212d2`); independent replay closure promoted in review `16.8.50`. |
...
| ASB-RQ-118 | strict governed outlet must remain actor-bound coherent: explicit actor context cannot emit replies for historical/non-current identity bindings, and strict send-time evidence must be file-backed live payload | actor-bound current-binding mismatch guard in compose/first-line validators + strict inline-evidence rejection + actor-bound telemetry surfaces | P0 | DONE | implementation landed in `FIX-061` (`119a421`) and independent re-audit promoted in review `16.8.84`: actor-bound mismatch is fail-closed (`IP-ASB-STAMP-SESSION-005`) and strict inline reply-text evidence is denied (`IP-ASB-STAMP-SESSION-002`) with non-governed bypass guard retained (`IP-ASB-STAMP-SESSION-004`). |
...
| ASB-RQ-044 / ASB-RQ-045 | `SPEC_READY -> GATE_READY` | protocol validators + six-surface wiring audit-passed (`P0-D/E`, review ledger `16.6.1~16.6.2`) |
| ASB-RQ-046 | `SPEC_READY -> IMPL_READY (BLOCKED_BY_AUDIT, P0)` | `validate_protocol_data_sanitization_boundary` + `FIX-047` path-context false-positive hardening landed; implementation replay passed for path-vs-real-sensitive regression pair, independent re-audit closure pending (`review 16.8.38`) |
...
| ASB-RQ-067 | `SPEC_READY -> GATE_READY (P0)` | independent replay closed in review `16.8.53` (`FIX-021` strict pass/fail + inspection warn semantics verified) |
| ASB-RQ-068 | `SPEC_READY -> GATE_READY (P0)` | independent replay closed in review `16.8.53` (`FIX-024` send-time reply-file positive + missing-header fail-closed verified) |
| ASB-RQ-069 | `SPEC_READY -> GATE_READY (P1)` | independent replay closed in review `16.8.53` (`FIX-025/026` instance/protocol routing + pass-through telemetry verified) |
| ASB-RQ-070 | `SPEC_READY -> GATE_READY (P0)` | independent replay closed in review `16.8.53` (`FIX-027` default instance + protocol-trigger + regression samples all passed) |
| ASB-RQ-071 / ASB-RQ-072 / ASB-RQ-073 / ASB-RQ-074 | `SPEC_READY -> GATE_READY (P0)` | independent replay closed in review `16.8.53` (`FIX-028` positive + MB001/MB003 negative paths verified, commit backfilled to `48e5445`) |
...
| ASB-RQ-090 / ASB-RQ-091 / ASB-RQ-092 / ASB-RQ-093 | `SPEC_READY -> GATE_READY (P0)` | `FIX-033` implementation (`0d7ebc7`) + lane propagation patch (`913973a`) + replay/doc closure (`9d830d8`, review `16.8.27`) closed prior `IP-LAYER-GATE-001` mismatch path; scope-limited instance operational Full-Go replay is documented in review `16.8.42` (non-status-changing evidence addendum) |
...
| ASB-RQ-109 / ASB-RQ-110 | `NEW -> GATE_READY (P0)` | implementation landed in `f5363e5` (`FIX-049/050`) and independent replay closed in review `16.8.55` (synthetic/no-guard fail-closed + live-guard pass, plus init/update pre-mutation gate receipts and report schema fields). |
...
| ASB-RQ-118 | `NEW -> DONE (P0)` | `FIX-061` implementation landed (`119a421`) and independent re-audit promoted in review `16.8.84`; actor-bound mismatch fail-closed + strict inline evidence rejection + non-governed bypass guard replay are accepted. |

### 6.4B Independent re-audit closure delta snapshot (2026-03-03)

This delta is authoritative for IDs listed below and supersedes earlier `6.4A`
phrasing that kept them in `IMPL_READY (BLOCKED_BY_AUDIT)`.
Historical implementation notes remain preserved in 6.4/6.4A.

| Requirement ID | Status delta | Evidence pointer |
| --- | --- | --- |
| ASB-RQ-046 | `IMPL_READY (BLOCKED_BY_AUDIT, P0) -> GATE_READY (P0)` | independent replay confirms `protocol_data_sanitization_boundary_status=PASS_REQUIRED` in this-round full-scan (`/tmp/reaudit_v15_fullscan_custom_live.json`), review `16.8.50` |
| ASB-RQ-075..089 | `IMPL_READY (BLOCKED_BY_AUDIT, P0) -> GATE_READY (P0)` | independent replay promotes `FIX-029..032` to `PASS` with full-scan + targeted chain validation (`/tmp/reaudit_v15_fullscan_custom_live.json`, review `16.8.50`) |
| ASB-RQ-094..096 | `IMPL_READY (BLOCKED_BY_AUDIT, P0) -> GATE_READY (P0)` | independent replay promotes `FIX-034..036` to `PASS`; lane-routing/run-pinned/hermetic checks pass in this-round replay bundle, review `16.8.50` |
| ASB-RQ-098 | `IMPL_READY (BLOCKED_BY_AUDIT, P1) -> GATE_READY (P1)` | strict update replay confirms no stale-only overreach (`phase_a_refresh_applied=false`) and retained `baseline_mode_violation` evidence (`IP-PBL-006`) remains fail-closed; review `16.8.50` |
| ASB-RQ-099..104 | `IMPL_READY (BLOCKED_BY_AUDIT, P0/P1) -> GATE_READY (P0/P1)` | independent replay promotes `FIX-039..044` to `PASS` including correlation path + invalid source-layer fail-closed + lock->exit->restore lifecycle (`/tmp/reaudit_fix04*.json`), review `16.8.50` |
| ASB-RQ-107..108 | `IMPL_READY (BLOCKED_BY_AUDIT, P0/P1) -> GATE_READY (P0/P1)` | independent replay promotes `FIX-048` to `PASS`: neutral scaffold zero legacy terms + blocker taxonomy canonical/legacy/invalid triad, review `16.8.50` |
| ASB-RQ-111..114 | `IMPL_READY (BLOCKED_BY_AUDIT, P0/P1) -> GATE_READY (P0/P1)` | independent replay promotes `FIX-051..054` to `PASS`: writeback/sidecar CWD invariance, coverage normalization, and send-time missing-head fail-closed, review `16.8.50` |

Historical release boundary at `6.4B` snapshot time:

1. `v1.5` remained `NO_GO` at that time because section `6.5` unlock formula was not yet satisfied.
2. This snapshot is preserved as historical progression evidence and is superseded for current release arithmetic by `6.4J`.

### 6.4C DONE promotion snapshot (2026-03-04, auditor closure batch-A)

Promotion rule used in this batch:

1. Promote `P0` rows from `GATE_READY/VERIFIED -> DONE` only when both are true:
   - requirement row already contains independent replay closure evidence anchor; and
   - corresponding review FIX row is `DONE/PASS`.
2. No row with `SPEC_READY` or `IMPL_READY (BLOCKED_BY_AUDIT)` is promoted in this batch.

Promoted in this batch (`37` rows):
...
Release boundary after this batch:

1. `D6` remains `LOCKED` because section `6.5` still requires all `P0` rows in section `6.4` to be `DONE`.
2. Remaining non-`DONE` `P0` rows are retained as-is pending implementation completion and/or independent re-audit closure.

### 6.4D DONE promotion snapshot (2026-03-04, auditor closure batch-B)

Promotion rule used in this batch:

1. Promote `P0` rows from `IMPL_READY (BLOCKED_BY_AUDIT) -> DONE` only when both are true:
   - corresponding fix summary row in review is `DONE/PASS`; and
   - replay evidence is explicitly promoted in review closure window (`16.8.50` plus follow-up windows where applicable).
2. `SPEC_READY` rows remain untouched in this batch.

Promoted in this batch (`26` rows):
...
1. `P0_TOTAL=87`, `P0_DONE=63`, `P0_NOT_DONE=24`.
2. Remaining `P0_NOT_DONE` rows are all `SPEC_READY` (`ASB-RQ-001..007`, `009`, `014..016`, `018..021`, `028..036`).
3. `D6` remains `LOCKED` until those `SPEC_READY` rows are implemented and then promoted to `DONE` via the same 6.5 formula.

### 6.4E DONE/IMPL promotion snapshot (2026-03-04, architect closure batch-C + FIX-059 intake)

Promotion rule used in this batch:

1. `SPEC_READY -> DONE` is allowed only for rows that already have:
   - replay-closed implementation chains recorded in review (`FIX` row `DONE/PASS`), and
   - existing governance delta evidence in 6.4A/B that is now normalized into 6.4 main table.
2. `SPEC_READY -> IMPL_READY (BLOCKED_BY_AUDIT)` is used when implementation lands this round, but independent re-audit is still pending.

Batch-C normalization (`21` rows, `SPEC_READY -> DONE`):
...
New implementation intake (`3` rows, `SPEC_READY -> IMPL_READY (BLOCKED_BY_AUDIT)`):

1. `ASB-RQ-014` (`FIX-059`, `2a8c3ee`: actor-risk profile coverage + machine-counted quartet)
2. `ASB-RQ-015` (`FIX-059`, `2a8c3ee`: heal replay deterministic refs + closure validator)
3. `ASB-RQ-016` (`FIX-059`, `2a8c3ee`: explicit execution-report binding in readiness/e2e closure checks)
...
2. Remaining `P0_NOT_DONE` set:
   - `ASB-RQ-117` (`IMPL_READY (BLOCKED_BY_AUDIT)`, independent promotion pending).
3. `D6` remains `LOCKED` until `ASB-RQ-117` is promoted to `DONE` under section `6.5` formula.

### 6.4H Independent re-audit promotion snapshot (`FIX-060`, 2026-03-04)

Snapshot decision:

1. Promote `ASB-RQ-117` from `IMPL_READY (BLOCKED_BY_AUDIT)` to `DONE` based on independent re-audit closure in review `16.8.82`.
2. This snapshot supersedes `6.4G` for current unlock arithmetic while preserving `6.4G` as historical pre-promotion state.

Release boundary after promotion:

1. `P0_TOTAL=88`, `P0_DONE=88`, `P0_NOT_DONE=0`.
...
### 6.4J Independent re-audit promotion snapshot (`FIX-061`, 2026-03-04)

Snapshot decision:

1. Promote `ASB-RQ-118` from `IMPL_READY (BLOCKED_BY_AUDIT)` to `DONE` based on independent re-audit closure in review `16.8.84`.
2. This snapshot supersedes `6.4I` for current unlock arithmetic while preserving `6.4I` as historical recurrence-intake state.

Release boundary after promotion:

1. `P0_TOTAL=89`, `P0_DONE=89`, `P0_NOT_DONE=0`.
2. Remaining `P0_NOT_DONE` set: none.
...
2. `v1.5` is `NO_GO` until the exact 6.5 unlock formula is satisfied; current status is `GO` because `6.4J` yields `unlock_allowed=true`.
...
2. Any future "identity changed during command execution" claim must replay against the listed anchors first (baseline + residual closure set).
...
### 6.6H Headstamp recurrence hard boundary (`review 16.8.79` intake, `16.8.82` promotion, `16.8.83/16.8.84` actor-bound closure)
...
7. Historical follow-up intake (`16.8.83`) introduced `ASB-RQ-118` as release-blocking `P0`.
8. Current authoritative state (`16.8.84`): `ASB-RQ-118` is independently promoted to `DONE`.
...
1. This lane is tracked as `ASB-RQ-067` (`P0`, `GATE_READY`); replay closure is audit-confirmed in review `16.8.53`.
