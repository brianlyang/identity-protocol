...
| ASB-RQ-109 | strict send-time/first-line gate must validate real outbound reply payload and reject synthetic stamp-only evidence sources; missing outlet guard is fail-closed | `scripts/validate_send_time_reply_gate.py`, `scripts/validate_reply_identity_context_first_line.py`, runtime reply outlet adapter (new), readiness/e2e/full-scan/three-plane evidence-mode telemetry | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | implementation landed in `FIX-049` (`f5363e5`): strict synthetic-evidence fail-closed (`IP-ASB-STAMP-SESSION-002`), missing outlet guard fail-closed (`IP-ASB-STAMP-SESSION-003`), and outlet telemetry wiring across compose/readiness/e2e/full-scan/three-plane; replay intake recorded in review `16.8.54`, independent audit promotion pending |
| ASB-RQ-110 | initialization/update mutation flow must enforce `header-first + scaffold-consent + mutation-plan-disclosure` before first write, and cannot bypass these gates through profile-local required-check omissions | mutation executor preflight (`execute_identity_upgrade.py` / creator update path), scaffold writer, gate telemetry collectors, readiness/e2e replay adapters | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | implementation landed in `FIX-050` (`f5363e5`): init/update pre-mutation gate receipts + fail-closed codes (`IP-EXEC-ORDER-001/002/003`) + execution-report schema fields (`header_first_gate_status`, `scaffold_consent_gate_status`, `mutation_plan_disclosed`, `planned_files`, `pre_mutation_gate_*`, `why_now`); replay intake recorded in review `16.8.54`, independent audit promotion pending |
...
| ASB-RQ-109 / ASB-RQ-110 | `NEW -> IMPL_READY (BLOCKED_BY_AUDIT, P0)` | implementation landed in `f5363e5` (`FIX-049/050`) with strict synthetic-evidence fail-closed + outlet guard enforcement and pre-mutation execution-order gating receipts/schema fields; replay intake recorded in review `16.8.54`, independent audit promotion pending |
...
2. Live replay still exposes environment/auth blocker `IP-CAP-003`; this is a release-plane blocker, not a protocol implementation re-audit blocker.
