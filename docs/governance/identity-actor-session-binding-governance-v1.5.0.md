...
| ASB-RQ-046 | protocol data sanitization boundary prevents tenant/business scenario leakage into protocol SSOT layer | `validate_protocol_data_sanitization_boundary` (new), governance/review document checks | P0 | IMPL_READY (BLOCKED_BY_AUDIT) | implementation landed, but project-scope re-audit still shows residual `IP-DSN-002` false-positive boundary (`review 16.8.36`) |
...
| ASB-RQ-046 | `SPEC_READY -> IMPL_READY (BLOCKED_BY_AUDIT, P0)` | `validate_protocol_data_sanitization_boundary` landed, but project-scope re-audit still has residual `IP-DSN-002` pending closure (`review 16.8.36`) |
