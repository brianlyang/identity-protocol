...
REQUIRED_PROGRESS_ACCOUNTING_FIELDS = (
...
DOC_REL_PATHS = {
    "governance": "docs/governance/identity-result-only-progress-accounting-governance-v1.6.x.md",
    "review": "docs/review/protocol-remediation-audit-ledger-v1.6.x-result-only-progress-accounting.md",
}
...
def validate_document_text(label: str, text: str) -> tuple[list[str], dict[str, Any] | None]:
    ...
    for field_name in REQUIRED_PROGRESS_ACCOUNTING_FIELDS:
        if not re.search(rf"\b{re.escape(field_name)}\b", text):
            errors.append(f"{label}:missing_progress_accounting_field:{field_name}")
    try:
...
def validate_contract_documents(repo_root: str | Path | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "required_progress_accounting_fields": list(REQUIRED_PROGRESS_ACCOUNTING_FIELDS),
        "errors": errors,
    }
