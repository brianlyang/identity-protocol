...
def _run_validator(*, repo_root: Path, catalog_path: Path, identity_id: str) -> tuple[int, dict[str, Any]]:
    script = (repo_root / "scripts" / "validate_identity_communication_transport.py").resolve()
    proc = subprocess.run(
        [
            "python3",
            str(script),
...
def main() -> int:
    ...
    for catalog_path in dedup:
        ...
        for row in _iter_catalog_rows(catalog_path=catalog_path):
            ...
            transport_status = str(validator_payload.get("identity_communication_transport_status", "")).strip().upper()
            row_state = {
                "identity_id": identity_id,
                "catalog_path": str(catalog_path),
                "validator_rc": validator_rc,
                "identity_communication_transport_status": transport_status,
                "communication_contract_status": str(validator_payload.get("communication_contract_status", "")).strip().upper(),
                "communication_runtime_roots_status": str(validator_payload.get("communication_runtime_roots_status", "")).strip().upper(),
                "handoff_transport_status": str(validator_payload.get("handoff_transport_status", "")).strip().upper(),
                "collaboration_transport_status": str(validator_payload.get("collaboration_transport_status", "")).strip().upper(),
                "protocol_feedback_reply_transport_status": str(validator_payload.get("protocol_feedback_reply_transport_status", "")).strip().upper(),
                "protocol_feedback_inbox_transport_status": str(validator_payload.get("protocol_feedback_inbox_transport_status", "")).strip().upper(),
                "protocol_feedback_atomic_transport_status": str(validator_payload.get("protocol_feedback_atomic_transport_status", "")).strip().upper(),
                "broadcast_transport_status": str(validator_payload.get("broadcast_transport_status", "")).strip().upper(),
                "missing_runtime_roots": [str(item).strip() for item in (validator_payload.get("missing_runtime_roots") or []) if str(item).strip()],
                "stale_reasons": [str(item).strip() for item in (validator_payload.get("stale_reasons") or []) if str(item).strip()],
                "status": STATUS_PASS_REQUIRED if validator_rc == 0 and transport_status == STATUS_PASS_REQUIRED else STATUS_FAIL_REQUIRED,
            }
    ...
    payload = {
        "identity_communication_transport_closure_status": status,
        "error_code": "" if status == STATUS_PASS_REQUIRED else ERR_COMMUNICATION_MIGRATION_INVALID,
