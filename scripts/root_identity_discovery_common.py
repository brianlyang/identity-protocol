...
IDENTITY_DISCOVERY_COMPLETENESS_SECTION_MARKER = "## Root identity-discovery completeness discipline"
...
@dataclass(frozen=True)
class IdentityDiscoveryCompletenessRow:
    order: int
    completeness_id: str
    contract_phrase: str
...
@dataclass(frozen=True)
class IdentityDiscoveryCompletenessSurfaceRow:
    order: int
    contract_phrase: str


@dataclass(frozen=True)
class IdentityDiscoveryCompletenessSurface:
    rel_path: str
    rows: tuple[IdentityDiscoveryCompletenessSurfaceRow, ...]
...
def identity_discovery_completeness_rows_from_doc(
    doc: Mapping[str, Any],
) -> tuple[IdentityDiscoveryCompletenessRow, ...]:
    rows = doc.get("identity_discovery_completeness_rows")
    if not isinstance(rows, list):
        return ()
    out: list[IdentityDiscoveryCompletenessRow] = []
    for row in rows:
        if not isinstance(row, dict):
            continue
        ...
        if order <= 0 or not completeness_id or not contract_phrase:
            continue
        out.append(
            IdentityDiscoveryCompletenessRow(
                order=order,
...
def readme_identity_discovery_completeness_surface(
    repo_root: Path,
) -> IdentityDiscoveryCompletenessSurface:
    path = (repo_root / ROOT_PROTOCOL_README_REL_PATH).resolve()
    if not path.exists() or not path.is_file():
        return IdentityDiscoveryCompletenessSurface(
    ...
    section_found = False
    rows: list[IdentityDiscoveryCompletenessSurfaceRow] = []

    for line in lines:
        stripped = line.strip()
        if stripped == IDENTITY_DISCOVERY_COMPLETENESS_SECTION_MARKER:
            section_found = True
            continue
        ...
        if not match:
            continue
        rows.append(
            IdentityDiscoveryCompletenessSurfaceRow(
