...
echo "[1/10] validate protocol"
python3 scripts/validate_identity_protocol.py

echo "[2/10] compile runtime brief"
python3 scripts/compile_identity_runtime.py

echo "[3/10] validate manifest semantics"
python3 scripts/validate_identity_manifest.py

echo "[4/10] test discovery contract"
python3 scripts/test_identity_discovery_contract.py >/tmp/identity_discovery_contract.protocol_repo.json

echo "[5/10] validate runtime ORRL contract"
python3 scripts/validate_identity_runtime_contract.py

echo "[6/10] validate update prereq baseline gate"
python3 scripts/validate_identity_upgrade_prereq.py --identity-id store-manager

echo "[7/10] validate update lifecycle contract (skill-style trigger/patch/validate/replay)"
python3 scripts/validate_identity_update_lifecycle.py --identity-id store-manager

echo "[8/10] validate trigger regression contract (positive/boundary/negative suites)"
python3 scripts/validate_identity_trigger_regression.py --identity-id store-manager

echo "[9/10] validate learning-loop linkage (reasoning + rulebook)"
...
echo "[10/10] ensure compile is stable"
