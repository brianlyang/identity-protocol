#!/usr/bin/env python3
"""Shared primitives for ISSUE-040B lane execution receipt contract validation."""
...
EXPECTED_LANE_ID = "issue_040b_lane_execution_receipt_contract_v1"
EXPECTED_GOVERNING_LAW = (
    "execution_receipt_required__handoff_state_must_be_durable_not_chat_native"
)
...
EXPECTED_COMMIT_GATE = "one isolated commit for ISSUE-040B only"
...
CONTRACT_START_MARKER = "<!-- lane-execution-receipt-contract:start -->"
CONTRACT_END_MARKER = "<!-- lane-execution-receipt-contract:end -->"
...
def extract_contract_payload(markdown: str) -> dict[str, Any]:
    ...
    if not match:
        raise ValueError("lane execution receipt contract block is missing")
    try:
        payload = json.loads(match.group(1))
    except json.JSONDecodeError as exc:
        raise ValueError(
            f"lane execution receipt contract block is not valid JSON: {exc}"
        ) from exc
    if not isinstance(payload, dict):
        raise ValueError("lane execution receipt contract payload must be a JSON object")
    return payload
...
def replace_contract_payload(markdown: str, payload: dict[str, Any]) -> str:
    ...
    if not pattern.search(markdown):
        raise ValueError("cannot replace missing lane execution receipt contract block")
...
def validate_expected_contract(payload: dict[str, Any]) -> list[str]:
    ...
    if payload.get("lane_id") != EXPECTED_LANE_ID:
        errors.append("lane_id does not match canonical ISSUE-040B value")
    if payload.get("governing_law") != EXPECTED_GOVERNING_LAW:
        errors.append("governing_law does not match canonical ISSUE-040B value")
    if payload.get("fixed_write_set") != EXPECTED_FIXED_WRITE_SET:
        errors.append("fixed_write_set does not match canonical ISSUE-040B write set")
    ...
    if payload.get("next_exact_action") != EXPECTED_NEXT_EXACT_ACTION:
        errors.append("next_exact_action does not match canonical ISSUE-040B action bundle")
    if payload.get("validation_bundle") != EXPECTED_VALIDATION_BUNDLE:
        errors.append("validation_bundle does not match canonical ISSUE-040B validation bundle")
    if payload.get("reopen_triggers") != EXPECTED_REOPEN_TRIGGERS:
        errors.append("reopen_triggers does not match the closed ISSUE-040B trigger set")
    if payload.get("commit_gate") != EXPECTED_COMMIT_GATE:
        errors.append("commit_gate does not match the isolated ISSUE-040B commit gate")
    return errors
