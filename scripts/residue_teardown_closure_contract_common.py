...
DOC_REL_PATHS = {
    "governance": "docs/governance/identity-residue-teardown-closure-governance-v1.6.x.md",
    "review": "docs/review/protocol-remediation-audit-ledger-v1.6.x-residue-teardown-closure.md",
}
...
def validate_document_text(label: str, text: str) -> tuple[list[str], dict[str, Any] | None]:
    ...
    for field_name in REQUIRED_TEARDOWN_FIELDS:
        if field_name not in text:
            errors.append(f"{label}:missing_teardown_field:{field_name}")
    try:
...
def validate_contract_documents(repo_root: str | Path | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "required_teardown_fields": list(REQUIRED_TEARDOWN_FIELDS),
        "errors": errors,
    }
