#!/usr/bin/env python3
"""Shared primitives for ISSUE-040A lane card handoff contract validation."""
...
EXPECTED_LANE_ID = "issue_040a_lane_card_handoff_contract_v1"
EXPECTED_GOVERNING_LAW = "chat_navigation_only__no_card_no_handoff_no_takeover"
...
EXPECTED_COMMIT_GATE = "one isolated commit for ISSUE-040A only"
...
CONTRACT_START_MARKER = "<!-- lane-card-handoff-contract:start -->"
CONTRACT_END_MARKER = "<!-- lane-card-handoff-contract:end -->"
...
def extract_contract_payload(markdown: str) -> dict[str, Any]:
    ...
    if not match:
        raise ValueError("lane card handoff contract block is missing")
    try:
        payload = json.loads(match.group(1))
    except json.JSONDecodeError as exc:
        raise ValueError(f"lane card handoff contract block is not valid JSON: {exc}") from exc
    if not isinstance(payload, dict):
        raise ValueError("lane card handoff contract payload must be a JSON object")
    return payload
...
def replace_contract_payload(markdown: str, payload: dict[str, Any]) -> str:
    ...
    if not pattern.search(markdown):
        raise ValueError("cannot replace missing lane card handoff contract block")
...
def validate_expected_contract(payload: dict[str, Any]) -> list[str]:
    ...
    if payload.get("lane_id") != EXPECTED_LANE_ID:
        errors.append("lane_id does not match canonical ISSUE-040A value")
    if payload.get("governing_law") != EXPECTED_GOVERNING_LAW:
        errors.append("governing_law does not match canonical ISSUE-040A value")
    if payload.get("fixed_write_set") != EXPECTED_FIXED_WRITE_SET:
        errors.append("fixed_write_set does not match canonical ISSUE-040A write set")
    ...
    if payload.get("next_exact_action") != EXPECTED_NEXT_EXACT_ACTION:
        errors.append("next_exact_action does not match canonical ISSUE-040A action bundle")
    if payload.get("validation_bundle") != EXPECTED_VALIDATION_BUNDLE:
        errors.append("validation_bundle does not match canonical ISSUE-040A validation bundle")
    if payload.get("reopen_triggers") != EXPECTED_REOPEN_TRIGGERS:
        errors.append("reopen_triggers does not match the closed ISSUE-040A trigger set")
    if payload.get("commit_gate") != EXPECTED_COMMIT_GATE:
        errors.append("commit_gate does not match the isolated ISSUE-040A commit gate")
    return errors
