    ...
    if not stale_reasons:
        ...
        if not contract_path.exists() or not contract_path.is_file():
            ...
            required_markers = tuple(
                str(item or "").strip() for item in responsibility_doc.get("contract_required_markers") if str(item or "").strip()
            )
            ...
            for row in layer_rows:
                for marker in find_missing_markers(contract_text, (row.contract_heading,)):
                    contract_marker_violations.append({"field": "contract_file", "reason": "layer_heading_missing", "marker": marker})
