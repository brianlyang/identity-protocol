...
CURRENT_TRUTH_EPISTEMOLOGY_COMPLETENESS_SECTION_MARKER = "## Root current-truth epistemology completeness discipline"
...
@dataclass(frozen=True)
class CurrentTruthEpistemologyCompletenessRow:
    order: int
    completeness_id: str
    contract_phrase: str
...
@dataclass(frozen=True)
class CurrentTruthEpistemologyCompletenessSurfaceRow:
    order: int
    contract_phrase: str


@dataclass(frozen=True)
class CurrentTruthEpistemologyCompletenessSurface:
    rel_path: str
    rows: tuple[CurrentTruthEpistemologyCompletenessSurfaceRow, ...]
...
def current_truth_epistemology_completeness_rows_from_doc(
    doc: Mapping[str, Any],
) -> tuple[CurrentTruthEpistemologyCompletenessRow, ...]:
    rows = doc.get("current_truth_epistemology_completeness_rows")
    if not isinstance(rows, list):
        return ()
    out: list[CurrentTruthEpistemologyCompletenessRow] = []
    for row in rows:
        if not isinstance(row, dict):
            continue
        ...
        if order <= 0 or not completeness_id or not contract_phrase:
            continue
        out.append(
            CurrentTruthEpistemologyCompletenessRow(
                order=order,
...
def readme_current_truth_epistemology_completeness_surface(
    repo_root: Path,
) -> CurrentTruthEpistemologyCompletenessSurface:
    path = (repo_root / ROOT_PROTOCOL_README_REL_PATH).resolve()
    if not path.exists() or not path.is_file():
        return CurrentTruthEpistemologyCompletenessSurface(
    ...
    section_found = False
    rows: list[CurrentTruthEpistemologyCompletenessSurfaceRow] = []

    for line in lines:
        stripped = line.strip()
        if stripped == CURRENT_TRUTH_EPISTEMOLOGY_COMPLETENESS_SECTION_MARKER:
            section_found = True
            continue
        ...
        if not match:
            continue
        rows.append(
            CurrentTruthEpistemologyCompletenessSurfaceRow(
