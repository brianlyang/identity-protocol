...
mutate_probe_literal "${GOVERNANCE_SHADOW_PATH}" "${philosophy_anchor_marker}" "identity/protocol/IDENTITY_PROTOCOL_DESIGN_OUTLINE.md"
mutate_probe_literal "${GOVERNANCE_SHADOW_PATH}" "${protocol_anchor_marker}" "identity/protocol/IDENTITY_PROTOCOL_CORE.md"
mutate_probe_literal "${REVIEW_SHADOW_PATH}" "${runtime_anchor_marker}" "identity/protocol/IDENTITY_RUNTIME_REPORT.md"

printf '[RUN] negative release-closure boundary doc-reference bundle validation\n'
if python3 "${REPO_ROOT}/scripts/validate_v16x_release_closure_boundary.py" --repo-root "${SHADOW_ROOT}" --json-only > "${NEGATIVE_JSON}"; then
  echo '[FAIL] negative release-closure boundary doc-reference bundle probe must fail'
  exit 1
fi
...
if positive.get("v16x_release_closure_boundary_status") != "PASS_REQUIRED":
    raise SystemExit("positive release-closure boundary doc-reference bundle status must PASS_REQUIRED")
if negative.get("v16x_release_closure_boundary_status") != "FAIL_REQUIRED":
    raise SystemExit("negative release-closure boundary doc-reference bundle status must FAIL_REQUIRED")

reasons = set(negative.get("stale_reasons") or [])
expected_reasons = {
    "governance_doc_missing_philosophy_anchor",
    "governance_doc_missing_protocol_anchor",
    "review_doc_missing_runtime_anchor",
}
missing = sorted(expected_reasons - reasons)
if missing:
    raise SystemExit(
        "negative release-closure boundary doc-reference bundle probe is missing expected stale reasons: "
        + ", ".join(missing)
    )
PY

echo "[PASS] v1.6.x release closure boundary doc-reference bundle probes passed"
