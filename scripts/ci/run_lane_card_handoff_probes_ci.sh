...
PROBE_ROOT="${TMPDIR}/lane-card-handoff-contract-probes"
rm -rf "${PROBE_ROOT}"
mkdir -p "${PROBE_ROOT}"

GOV_DOC="${REPO_ROOT}/docs/governance/identity-lane-card-handoff-governance-v1.6.x.md"
REVIEW_DOC="${REPO_ROOT}/docs/review/protocol-remediation-audit-ledger-v1.6.x-lane-card-handoff.md"
VALIDATOR="${REPO_ROOT}/scripts/validate_lane_card_handoff_contract.py"
...
if payload.get("lane_card_handoff_contract_status") != "PASS_REQUIRED":
    raise SystemExit(f"expected PASS_REQUIRED, got {payload.get('lane_card_handoff_contract_status')}")
...
payload = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
if payload.get("lane_card_handoff_contract_status") != "FAIL_REQUIRED":
    raise SystemExit(f"expected FAIL_REQUIRED, got {payload.get('lane_card_handoff_contract_status')}")
...
from lane_card_handoff_contract_common import extract_contract_payload, replace_contract_payload
...
for target in targets:
    ...
    if mode == "missing_commit_gate":
        ...
        payload["governing_law"] = "chat_navigation_only__drifted"
...
phrase = "Chat is navigation-only and is not a durable handoff state."
replacement = "Chat remains available for operator navigation."
...
echo "PASS: lane card handoff probes"
