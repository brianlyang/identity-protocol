...
printf '[RUN] positive release-closure boundary narrative-bundle validation\n'
...
printf '[RUN] negative release-closure boundary narrative-bundle validation\n'
if python3 "${REPO_ROOT}/scripts/validate_v16x_release_closure_boundary.py" --repo-root "${SHADOW_ROOT}" --json-only > "${NEGATIVE_JSON}"; then
  echo '[FAIL] negative release-closure boundary narrative-bundle probe must fail'
  exit 1
fi
...
if positive.get("v16x_release_closure_boundary_status") != "PASS_REQUIRED":
    raise SystemExit("positive release-closure boundary narrative-bundle status must PASS_REQUIRED")
if negative.get("v16x_release_closure_boundary_status") != "FAIL_REQUIRED":
    raise SystemExit("negative release-closure boundary narrative-bundle status must FAIL_REQUIRED")

reasons = set(negative.get("stale_reasons") or [])
expected_reasons = {
    "governance_doc_missing_active_report_pointer_locality_marker:latest_identity_upgrade_report()",
    "governance_doc_missing_strict_live_contract_resolution_marker:sample-green fail-close",
    "review_doc_missing_strict_live_active_pointer_locality_marker:resolve_active_execution_context()",
    "review_doc_missing_weak_live_pointer_absorption_marker:current_run_pointer_resolution_mode",
    "review_doc_missing_execution_report_selection_convergence_marker:execution_report_selection_common.py",
}
missing = sorted(expected_reasons - reasons)
if missing:
    raise SystemExit(
        "negative release-closure boundary narrative-bundle probe is missing expected stale reasons: "
        + ", ".join(missing)
    )
PY

echo "[PASS] v1.6.x release closure boundary narrative-bundle probes passed"
