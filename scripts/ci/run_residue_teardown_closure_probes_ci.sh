...
mkdir -p "$TMP_ROOT"
WORK_ROOT="$(mktemp -d "$TMP_ROOT/residue-teardown-probes.XXXXXX")"
trap 'rm -rf "$WORK_ROOT"' EXIT

VALIDATOR="$REPO_ROOT/scripts/validate_residue_teardown_closure_contract.py"
GOV_DOC_REL="docs/governance/identity-residue-teardown-closure-governance-v1.6.x.md"
REV_DOC_REL="docs/review/protocol-remediation-audit-ledger-v1.6.x-residue-teardown-closure.md"
...
mutate_replace \
  ...
  "residue_teardown_owner_removed"
...
mutate_replace \
  ...
  "closure_incomplete_if_residue_teardown_missing" \
  "closure_incomplete_if_residue_teardown_optional"
...
mutate_replace \
  ...
  "closure may proceed without teardown receipt"
...
mutate_replace \
  ...
  "one isolated commit for ISSUE-041A only" \
  "multi-commit allowed for ISSUE-041A"
...
mutate_replace \
  ...
  "Fail-close applies whenever residue teardown may be uncertain."
