...
COMPLETENESS_ROW_ORDER_REPO="${TMP_ROOT}/decision-evidence-completeness-row-order-noncontiguous-repo"
mirror_repo "${COMPLETENESS_ROW_ORDER_REPO}"
python3 - <<'PY' "${COMPLETENESS_ROW_ORDER_REPO}/identity/protocol/mappings/root-decision-evidence-admissibility.v1.yaml"
import pathlib
import sys
import yaml
...
path = pathlib.Path(sys.argv[1])
doc = yaml.safe_load(path.read_text(encoding="utf-8"))
target_id = "congruent_decision_evidence_admissibility_row_family_totals"
for row in doc["decision_evidence_admissibility_completeness_rows"]:
    if row.get("completeness_id") == target_id:
        row["order"] = 6
        break
else:
    raise SystemExit("expected decision-evidence completeness row not found")
path.write_text(yaml.safe_dump(doc, sort_keys=False), encoding="utf-8")
PY

COMPLETENESS_ROW_ORDER_JSON="${TMP_ROOT}/decision-evidence-completeness-row-order-noncontiguous.json"
if python3 "${ROOT}/scripts/validate_protocol_root_decision_evidence_admissibility.py" \
  --repo-root "${COMPLETENESS_ROW_ORDER_REPO}" \
  --json-only >"${COMPLETENESS_ROW_ORDER_JSON}"; then
  echo "[FAIL] root decision-evidence admissibility validator unexpectedly passed completeness row order non-contiguous"
  exit 1
fi
...
assert any(
    row["field"] == "decision_evidence_admissibility_completeness_rows"
    and row["reason"] == "decision_evidence_admissibility_completeness_row_order_non_contiguous"
    for row in payload["structure_violations"]
), payload
assert any(
    row["reason"] == "decision_evidence_admissibility_completeness_row_order_mismatch"
...
assert_stale_reason_present "${COMPLETENESS_ROW_ORDER_JSON}" "structure_violation:decision_evidence_admissibility_completeness_rows:decision_evidence_admissibility_completeness_row_order_non_contiguous"
assert_stale_reason_present "${COMPLETENESS_ROW_ORDER_JSON}" "decision_evidence_admissibility_violation:decision_evidence_admissibility_completeness_rows:decision_evidence_admissibility_completeness_row_order_mismatch"
...
if python3 "${ROOT}/scripts/validate_protocol_root_decision_evidence_admissibility.py" \
  --repo-root "${DECISION_EVIDENCE_SURFACE_ORDER_REPO}" \
  --json-only >"${DECISION_EVIDENCE_SURFACE_ORDER_JSON}"; then
  echo "[FAIL] root decision-evidence admissibility validator unexpectedly passed completeness surface order drift"
  exit 1
fi

python3 - <<'PY' "${DECISION_EVIDENCE_SURFACE_ORDER_JSON}"
import json
import pathlib
import sys
...
assert any(
    row["field"] == "decision_evidence_admissibility_completeness_surface"
    and row["reason"] == "decision_evidence_admissibility_completeness_surface_order_mismatch"
    for row in payload["admissibility_violations"]
), payload
assert any(
    reason == "decision_evidence_admissibility_violation:decision_evidence_admissibility_completeness_surface:decision_evidence_admissibility_completeness_surface_order_mismatch"
    for reason in payload["stale_reasons"]
), payload
surface_row = next(
    row for row in payload["row_family_projection_rows"]
...
COMPLETENESS_SURFACE_ORDER_NONCONTIG_REPO="${TMP_ROOT}/decision-evidence-completeness-surface-order-noncontiguous-repo"
...
COMPLETENESS_SURFACE_ORDER_NONCONTIG_JSON="${TMP_ROOT}/decision-evidence-completeness-surface-order-noncontiguous.json"
if python3 "${ROOT}/scripts/validate_protocol_root_decision_evidence_admissibility.py" \
  --repo-root "${COMPLETENESS_SURFACE_ORDER_NONCONTIG_REPO}" \
  --json-only >"${COMPLETENESS_SURFACE_ORDER_NONCONTIG_JSON}"; then
  echo "[FAIL] root decision-evidence admissibility validator unexpectedly passed completeness surface order non-contiguous"
  exit 1
fi
...
assert any(
    row["field"] == "decision_evidence_admissibility_completeness_surface"
    and row["reason"] == "decision_evidence_admissibility_completeness_surface_order_non_contiguous"
    for row in payload["structure_violations"]
), payload
assert any(
    row["reason"] == "decision_evidence_admissibility_completeness_surface_order_mismatch"
    for row in payload["admissibility_violations"]
), payload
surface_row = next(
    row for row in payload["row_family_projection_rows"]
...
assert_stale_reason_present "${COMPLETENESS_SURFACE_ORDER_NONCONTIG_JSON}" "structure_violation:decision_evidence_admissibility_completeness_surface:decision_evidence_admissibility_completeness_surface_order_non_contiguous"
assert_stale_reason_present "${COMPLETENESS_SURFACE_ORDER_NONCONTIG_JSON}" "decision_evidence_admissibility_violation:decision_evidence_admissibility_completeness_surface:decision_evidence_admissibility_completeness_surface_order_mismatch"
