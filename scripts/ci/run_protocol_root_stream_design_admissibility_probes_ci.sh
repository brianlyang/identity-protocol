...
text = text.replace(
    ...
    "2. required question, admissibility-proof, admissibility-limit, outcome-class, and projection-surface rows must remain explicit as separate machine-readable families;\n"
...
SURFACE_ORDER_JSON="${TMP_ROOT}/stream-design-admissibility-completeness-surface-order-drift.json"
if python3 "${ROOT}/scripts/validate_protocol_root_stream_design_admissibility.py" \
  --repo-root "${SURFACE_ORDER_REPO}" \
  --json-only >"${SURFACE_ORDER_JSON}"; then
  echo "[FAIL] root stream-design admissibility validator unexpectedly passed completeness surface order drift"
  exit 1
fi
...
assert any(
    row["field"] == "stream_design_admissibility_completeness_surface"
    and row["reason"] == "stream_design_admissibility_completeness_surface_order_mismatch"
    for row in payload["admissibility_violations"]
), payload
assert any(
    row["rel_path"] == "identity/protocol/README.md"
    and row["reason"] == "required_marker_missing"
    and row["marker"] == "1. required question, admissibility-proof, admissibility-limit, outcome-class, and projection-surface rows must remain explicit as separate machine-readable families;"
    for row in payload["root_doc_anchor_violations"]
), payload
surface_row = next(
    row for row in payload["row_family_projection_rows"]
