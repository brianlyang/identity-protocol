...
printf '[RUN] negative release-closure summary foundational-bundle validation\n'
if python3 "${REPO_ROOT}/scripts/validate_v16x_release_closure_summary.py" --repo-root "${SHADOW_ROOT}" --json-only > "${NEGATIVE_JSON}"; then
  echo '[FAIL] negative release-closure summary foundational-bundle probe must fail'
  exit 1
fi
...
if positive.get("v16x_release_closure_summary_status") != "PASS_REQUIRED":
    raise SystemExit("positive release-closure summary foundational-bundle status must PASS_REQUIRED")
if negative.get("v16x_release_closure_summary_status") != "FAIL_REQUIRED":
    raise SystemExit("negative release-closure summary foundational-bundle status must FAIL_REQUIRED")

reasons = set(negative.get("stale_reasons") or [])
expected_reasons = {
    "philosophy_root_order_markers_missing",
    "summary_doc_missing_root_machine_runtime_closure_markers",
...
if missing:
    raise SystemExit(
        "negative release-closure summary foundational-bundle probe is missing expected stale reasons: "
        + ", ".join(missing)
    )
PY

echo "[PASS] v1.6.x release closure summary foundational-bundle probes passed"
