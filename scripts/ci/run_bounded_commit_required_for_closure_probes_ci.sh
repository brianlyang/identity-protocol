...
mkdir -p "$TMP_ROOT"
WORK_ROOT="$(mktemp -d "$TMP_ROOT/bounded-commit-required-for-closure-probes.XXXXXX")"
trap 'rm -rf "$WORK_ROOT"' EXIT

VALIDATOR="$REPO_ROOT/scripts/validate_bounded_commit_required_for_closure_contract.py"
GOV_DOC_REL="docs/governance/identity-bounded-commit-required-for-closure-governance-v1.6.x.md"
REV_DOC_REL="docs/review/protocol-remediation-audit-ledger-v1.6.x-bounded-commit-required-for-closure.md"
...
mutate_replace \
  ...
  "bounded_commit_status_removed"
run_expect_fail "$missing_field_dir" "missing required closure accounting field"
...
mutate_replace \
  ...
  "Closure claims without bounded commit or equivalent durable closure receipt evidence require manual interpretation."
