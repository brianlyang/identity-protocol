...
INTEGRATED_PROBE_COMMON="${REPO_ROOT}/scripts/release_closure_integrated_probe_common.py"
...
python3 "${INTEGRATED_PROBE_COMMON}" mutate --profile boundary --path "${GOVERNANCE_SHADOW_PATH}"
...
python3 "${INTEGRATED_PROBE_COMMON}" \
  assert \
  --profile boundary \
