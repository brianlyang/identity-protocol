...
mkdir -p "$TMP_ROOT"
WORK_ROOT="$(mktemp -d "$TMP_ROOT/result-only-progress-accounting-probes.XXXXXX")"
trap 'rm -rf "$WORK_ROOT"' EXIT

VALIDATOR="$REPO_ROOT/scripts/validate_result_only_progress_accounting_contract.py"
GOV_DOC_REL="docs/governance/identity-result-only-progress-accounting-governance-v1.6.x.md"
REV_DOC_REL="docs/review/protocol-remediation-audit-ledger-v1.6.x-result-only-progress-accounting.md"
...
mutate_replace \
  ...
  "durable_repo_write_status_removed"
run_expect_fail "$missing_field_dir" "missing required progress field"
...
mutate_replace \
  ...
  "Progress claims without durable repo output require manual interpretation."
