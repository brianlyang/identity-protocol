...
run_shadow_validator() {
  ...
  PYTHONPATH="${shadow_root}/scripts${PYTHONPATH:+:${PYTHONPATH}}" \
    python3 "${shadow_root}/scripts/validate_release_readiness_one_look_topology.py" \
...
python3 scripts/validate_release_readiness_one_look_topology.py --json-only >"${POSITIVE_JSON}"
...
mutate_probe_literal \
  ...
  'one_look_field="release_readiness_one_look_topology_probe_status"' \
...
restore_shadow_file "${TMP_ROOT}" "scripts/ci/run_release_readiness_one_look_topology_probes_ci.sh"
mutate_probe_literal \
  "${TMP_ROOT}/scripts/ci/run_release_readiness_one_look_topology_probes_ci.sh" \
...
print(json.dumps({
    "release_readiness_one_look_topology_probe_status": "PASS_REQUIRED",
