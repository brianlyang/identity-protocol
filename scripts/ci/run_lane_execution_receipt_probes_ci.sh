...
PROBE_ROOT="${TMPDIR}/lane-execution-receipt-contract-probes"
rm -rf "${PROBE_ROOT}"
mkdir -p "${PROBE_ROOT}"

GOV_DOC="${REPO_ROOT}/docs/governance/identity-lane-execution-receipt-governance-v1.6.x.md"
REVIEW_DOC="${REPO_ROOT}/docs/review/protocol-remediation-audit-ledger-v1.6.x-lane-execution-receipt.md"
VALIDATOR="${REPO_ROOT}/scripts/validate_lane_execution_receipt_contract.py"
...
if payload.get("lane_execution_receipt_contract_status") != "PASS_REQUIRED":
    raise SystemExit(
        f"expected PASS_REQUIRED, got {payload.get('lane_execution_receipt_contract_status')}"
    )
...
payload = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
if payload.get("lane_execution_receipt_contract_status") != "FAIL_REQUIRED":
    raise SystemExit(
        f"expected FAIL_REQUIRED, got {payload.get('lane_execution_receipt_contract_status')}"
    )
...
from lane_execution_receipt_contract_common import (
...
for target in targets:
    ...
    if mode == "missing_commit_gate":
        ...
        payload["governing_law"] = "execution_receipt_required__drifted"
        ...
        payload["next_exact_action"] = payload["next_exact_action"] + ["allow chat recap as fallback"]
...
phrase = "Execution receipt is required for handoff and continuation."
replacement = "Execution summary may remain chat-first."
...
echo "PASS: lane execution receipt probes"
