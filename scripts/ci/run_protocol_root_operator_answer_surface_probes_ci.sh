...
SURFACE_ORDER_REPO="${TMP_ROOT}/operator-answer-surface-completeness-surface-order-drift-repo"
...
text = text.replace(
    ...
    "2. required surface, answer-surface-stage, answer-surface-stage-surface, support-memory, support-limit, answer-claim-alignment, answer-claim-epistemic-alignment, answer-surface-proof, answer-surface-limit, boundary, and collapse rows must remain explicit as separate machine-readable families;\n"
...
SURFACE_ORDER_JSON="${TMP_ROOT}/operator-answer-surface-completeness-surface-order-drift.json"
if python3 "${ROOT}/scripts/validate_protocol_root_operator_answer_surface.py" \
  --repo-root "${SURFACE_ORDER_REPO}" \
  --json-only >"${SURFACE_ORDER_JSON}"; then
  echo "[FAIL] root operator answer-surface validator unexpectedly passed completeness surface order drift"
  exit 1
fi
...
assert any(
    row["rel_path"] == "identity/protocol/README.md"
    and row["reason"] == "required_marker_missing"
    and row["marker"] == "1. required surface, answer-surface-stage, answer-surface-stage-surface, support-memory, support-limit, answer-claim-alignment, answer-claim-epistemic-alignment, answer-surface-proof, answer-surface-limit, boundary, and collapse rows must remain explicit as separate machine-readable families;"
    for row in payload["root_doc_anchor_violations"]
), payload
surface_row = next(
    row for row in payload["row_family_projection_rows"]
