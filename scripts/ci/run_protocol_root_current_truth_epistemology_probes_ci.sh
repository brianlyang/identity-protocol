...
path = pathlib.Path(sys.argv[1])
text = path.read_text(encoding="utf-8")
old = "## Root current-truth epistemology completeness discipline"
new = "## Root current-truth epistemology discipline"
assert old in text, text
path.write_text(text.replace(old, new, 1), encoding="utf-8")
PY
...
assert any(
    ...
    and row["marker"] == "## Root current-truth epistemology completeness discipline"
