...
def _default_role_binding_sample(identity_id: str, role_type: str) -> dict:
    ...
    return {
        ...
        "runtime_bootstrap": {
            ...
            "evidence": f"identity/runtime/examples/{identity_id}-bootstrap-runtime-validation-sample.json",
        },
        "switch_guard": {
            "status": "PASS",
...
def main() -> int:
    ...
    role_binding_sample_path = Path("identity/runtime/examples") / f"identity-role-binding-{identity_id}-sample.json"
    role_type = str((current_task.get("identity_role_binding_contract") or {}).get("role_type", f"{identity_id}_runtime_role"))
    write_json(role_binding_sample_path, _default_role_binding_sample(identity_id, role_type))

    negative_role_binding_sample_path = (
        Path("identity/runtime/examples/role-binding")
    ...
    if not args.skip_bootstrap_check:
        ...
        if args.register:
            checks.append(["python3", "scripts/validate_identity_role_binding.py", "--identity-id", identity_id])
        for cmd in checks:
            print("$", " ".join(cmd))
