...
ERR_PREREQ_INVALID = "IP-ISLANE-001"
ERR_LANE_INVALID = "IP-ISLANE-002"
ERR_TARGET_NOT_FOUND = "IP-ISLANE-003"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Validate route-to-execution-lane admission governance.")
    ...
    lane_required = execution_lane_required(task_doc)
    payload: dict = {
        "identity_id": args.identity_id,
        ...
        "execution_lane_required": lane_required,
        "route_execution_lane_admission_status": STATUS_SKIPPED_NOT_REQUIRED,
    ...
    if not lane_required:
        payload["stale_reasons"] = ["execution_lane_contract_not_required"]
        _emit(payload, json_only=args.json_only)
        return 0
    ...
    if manifest_doc is None:
        ...
        payload["route_execution_lane_admission_status"] = STATUS_FAIL_REQUIRED
        payload["error_code"] = ERR_PREREQ_INVALID
        payload["stale_reasons"] = ["manifest_missing_for_execution_lane_admission"]
        _emit(payload, json_only=args.json_only)
        return 1
    ...
    payload["route_total_count"] = int(lane_validation.get("route_total_count", 0))
    payload["route_checked_count"] = int(lane_validation.get("route_checked_count", 0))
    payload["route_observed_count"] = int(lane_validation.get("route_observed_count", 0))
    payload["route_rows"] = list(lane_validation.get("route_rows") or [])
    payload["stale_reasons"] = list(lane_validation.get("stale_reasons") or [])
    ...
    if str(lane_validation.get("status", "")).strip() != STATUS_PASS_REQUIRED:
        payload["route_execution_lane_admission_status"] = (
            str(lane_validation.get("status", "")).strip() or STATUS_FAIL_REQUIRED
        )
        payload["error_code"] = ERR_LANE_INVALID
        _emit(payload, json_only=args.json_only)
        return 1

    payload["route_execution_lane_admission_status"] = STATUS_PASS_REQUIRED
    _emit(payload, json_only=args.json_only)
    return 0
